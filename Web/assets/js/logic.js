...
function globalCalculate(padre){
            ...
            var totalAnios = 0 , meses_total = 1;
            if ( periodo == "mes" && periodos >= 12 ){
                var anio_value  = 12,
                    totalAnios  = Math.round(periodos / anio_value),
                    meses_total = (periodos - anio_value); 
                    console.warn("totalAnios", totalAnios);
                    console.warn("meses_total", meses_total);
            }else if( periodo == "mes" && periodos < 11){
                    meses_total = periodos; 
            }


           
            var fecha = new Date(start_date_),
                anios = fecha.getFullYear() + totalAnios, 
                months = fecha.getMonth() + meses_total,
                utc = fecha.getUTCDate() ;

            var fecha_ = new Date(anios,months,utc);
                //end_date_.
            //var date_ = new Date()., new Date().getMonth()+meses_total, new Date().getUTCDate();
            console.warn( anios );
            console.warn( months );
            console.warn( utc );

            console.warn(anios+"-"+months+"-"+utc);

             end_date_.val(anios+"-"+months+"-"+utc );
                    ...
                    padre.find(".aditionals-elements").each(function(index){
                        var grupo = $(this).data('grupo'),
                            ...
                            if ( optionDataAdic != undefined ){
                                obtainUnitValues( padre, optionDataAdic, periodoText, periodos, cantidad, "optionDataAdic", cloneIndex );  
       ...
       function obtainUnitValues(padre, mainObj, searchBy, cantidad, periodos, identify, cloneI){
            //console.warn(cloneI);
        ...
        if ( TimePeriod != undefined && cantidadPeriodos != '' && PuestoDeTrabajo != '' && PuestoDeTrabajoCantidad != ''){
                        ...
                        var adiccionalTemplate = $("<div/>");
                            adiccionalTemplate.addClass("col-xl-12 container-aditionals");
                                    ...
                                    /* carHtml +=    '<div class="aditionalsElements ">';
                                         carHtml +=    '</div>';*/
                                     ...
                                     /*::::Elementos Adicionales::::*/
                                        $(this).find("#adicionalesOm .aditional_").each(function(){
                                            ...
                                            var paragraph = $("<p/>");
                                                paragraph.append('<h5 class="m-0 p-0">'+ElementoAdicionalNombre+'</h5><span>'+ElementoAdicionalUnitario+'</span>');
                                                tmpAditional.append(paragraph);

                                            adiccionalTemplate.append( tmpAditional );
                                        });
                                        console.info(adiccionalTemplate);
                                    template = $(template).append(adiccionalTemplate);       
                                /*$(template).append( adiccionalTemplate );
                                */
